...
def test_str_to_list():
    ...
    assert (
        response == result
    ), f"FAILED test_str_to_list param='{param}', assert {response} == {result}"
    print(f"[+] test param='{param}', result={result}")

    param = "999999    hhh"
    result = ["h", "h", "h"]
    assert (
        str_to_list(param) == result
    ), f"FAILED test_str_to_list param='{param}', assert {response} == {result}"
    print(f"[+] test param='{param}', result={result}")
...
def test_convert():
    ...
    assert (
        response == result
    ), f"FAILED test_convert param='{param}', assert {response} == {result}"
    print(f"[+] test param='{param}', result={result}")

    param = "CML"
    result = 950
    assert (
        convert(param) == result
    ), f"FAILED test_convert param='{param}', assert {response} == {result}"
    print(f"[+] test param='{param}', result={result}")
